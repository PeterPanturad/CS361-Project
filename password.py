...
def main(hexPasswords):
    ...
    for i  in range(n // 2):
        if query(bitArray, m, k, hexPasswords[i]) == True:
            trainingSetTrues += 1
        if query(bitArray, m, k, hexPasswords[i + 450000]) == True:
            querySetTrues += 1
    print(f"training trues: {trainingSetTrues}\nquery trues: {querySetTrues}")
